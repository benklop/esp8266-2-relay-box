...
ins_dep   = 5.0;     // insert depth (leaves 1mm floor at bottom)
boss_od   = 9;       // boss outer diameter
boss_h    = 3;       // boss protrusion above floor
// Total depth available: floor_t(3) + boss_h(3) = 6 mm
// Insert depth: 5 mm → 1 mm solid bottom remains ✓

// ── Terminal openings ──────────────────────────────────────────
term_a    = 30.5;    // opening at +Y end (centered)
term_b    = 15.5;    // opening at −Y end (centered)
term_bot  = 3;       // solid strip at bottom before opening

// ── Mounting tabs ──────────────────────────────────────────────
tab_ext   = 10;      // extension beyond box wall
tab_len   = 22;      // length along Y (centered on box)
tab_t     = 3;       // thickness (sits flush at box bottom)
...
lip_gap   = 0.3;     // radial clearance between lip and inner wall
...
// ── Derived ────────────────────────────────────────────────────
lip_ow    = inner_w - 2*lip_gap;       // lip outer W = 49.4
lip_ol    = inner_l - 2*lip_gap;       // lip outer L = 80.4
lip_iw    = lip_ow - 2*lip_wall;       // lip inner W = 46.4
lip_il    = lip_ol - 2*lip_wall;       // lip inner L = 77.4

// ═══════════════════════════════════════════════════════════════
// Modules
// ═══════════════════════════════════════════════════════════════

// Boss with heat-set insert hole.
// Placed at (x, y, 0); boss runs from Z=0 to Z=floor_t+boss_h,
// insert hole opens at top (accessible from inside box).
module boss_insert() {
    difference() {
        cylinder(d=boss_od, h=floor_t + boss_h);
        // Insert hole from top of boss downward
        translate([0, 0, floor_t + boss_h - ins_dep])
            cylinder(d=ins_d, h=ins_dep + 0.01);
    }
}

// Flat mounting tab (rectangular, Z-axis mounting hole)
module tab_shape() {
    difference() {
        cube([tab_ext, tab_len, tab_t]);
        // M3 clearance hole, centered in tab
...
module project_box() {
    union() {
        ...
        // PCB mounting bosses (union after difference so holes stay)
...
module lid() {
    difference() {
        union() {
            // Lid plate (Z=0 at bottom face of plate, sits on box rim)
            translate([-outer_w/2, -outer_l/2, 0])
...
// ═══════════════════════════════════════════════════════════════
// Scene — box at Z=0, lid exploded above for inspection
// ═══════════════════════════════════════════════════════════════

project_box();

translate([0, 0, outer_h + 12])
    lid();
