...
// ── M3 heat-set inserts / bosses (Tokai UD-43030, 3mm length) ──
// Insert OD = 4.2mm; hole is undersized so plastic displaces into knurls
...
ins_d      = 3.8;    // recommended hole Ø for UD-43030 (undersized vs 4.2mm OD)
...
boss_od    = 8;      // boss OD → (8-3.8)/2 = 2.1mm wall ≈ 5 perimeters @ 0.4mm
boss_h     = 3;      // boss height = insert length → insert sits flush with boss top

// ── Standoff + PCB stack ───────────────────────────────────────
standoff_h = 5;      // typical M3 male-female standoff height
pcb_t      = 1.6;   // standard FR4 PCB thickness

// Heights from box floor:
pcb_bot_z  = floor_t + boss_h + standoff_h;  // 11mm — bottom of PCB
pcb_top_z  = pcb_bot_z + pcb_t;              // 12.6mm — top of PCB

// Inner height = clearance above PCB top (component space)
inner_h    = 18;     // 17mm components + 1mm headroom
outer_h    = pcb_top_z + inner_h;  // ≈ 30.6mm total box height
